...
class FlowHttp extends EventEmitter{
	...
	buildSesssionCookie(req){
        ...
        if(!Cookie || !CookieSignature || !req.id)
            return false;

        let cookieName          = this.getHttpSessionCookieName();
        let signed              = 's:'+CookieSignature.sign(req.id, this.getHttpSessionSecret());

        return Cookie.serialize(cookieName, signed, req.id);
    }
	...
	init_socketio_handler(websocketPath, websocketMode) {
		...
		this.websockets = this.io.on('connection', async (conn)=>{
			...
			conn.on('data', (data)=>{
				...
				switch (event) {
					case 'auth::cookie': {
						this.getSocketSession(msg).then(sess=>{
							session = sess;
							let sessionCookie = this.buildSesssionCookie(conn);
