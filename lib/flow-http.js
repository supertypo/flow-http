...
class FlowHttp extends EventEmitter{
	...
	initSocketHandler(sockjs, websocketPath, websocketMode) {
		...
		this.websockets = this.io.on('connection', async (conn_impl)=>{
			...
			const ip = this.getIpFromHeaders(conn_impl.headers) || conn_impl.remoteAddress;
			//console.log(socket);
			const socket = {
				conn_impl,
				id : conn_impl.id,
				...
				emit(...args) {
					if(socket_is_alive)
						conn_impl.write(JSON.stringify(args));
				},
				publish(subject, data) {
					if(socket_is_alive)
						conn_impl.write(JSON.stringify(['publish',{ subject, data }]));
				},
			...
			socket.on('data', (data)=>{
				...
				if(event == 'auth.cookie') {
					...
					let req = {
						url: conn_impl.url,
						pathname: conn_impl.pathname,
						protocol: conn_impl.protocol,
						connection: conn_impl.connection,
						headers: conn_impl.headers
					};
					let res = { };
					res.writeHead = () => ({ });
					...
					this.expressSession(req, res, ()=> {
						...
						socket.emit('ready', {websocketMode});
						const msg = { socket, ip, session };
