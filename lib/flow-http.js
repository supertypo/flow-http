...
class FlowHttp extends EventEmitter{
	...
	constructor(appFolder, options={}){
		...
		this.socket = {
			...
			broadcast : (subject, msg) => {
				this.websocketMap.forEach((socket) => socket.write([subject,msg]));
			},
			subscribe : (subject) => {
		...
		return new Promise((resolve, reject)=>{
			...
			if(sockjs) {
				...
				if(this.config.websocketPath)
					this.initSocketHandler(sockjs, this.config.websocketPath, this.config.websocketMode || FlowHttp.MODE.RPC);
			}
	...
	initSocketHandler(sockjs, websocketPath, websocketMode) {
		this.sockjs = sockjs;
		...
		this.websockets = this.io.on('connection', async (socket)=>{
			//console.log(socket);
			let socket_is_alive = true;
			const ip = this.getIpFromHeaders(socket.headers) || socket.remoteAddress;
			const emit = (...args) => {
				if(socket_is_alive)
			...
			socket.on('data', (data)=>{
				...
				if(event == 'auth::cookie') {
					...
					res.setHeader = (name, value) => {
						if (name === 'Set-Cookie') {
							emit('auth::setcookie', { cookie: value });
						}
					}
					if (cookie !== null) {
					...
					if(websocketMode == FlowHttp.MODE.RPC || websocketMode == 'RPC') {
						switch (event) {

							case 'message': {
								try {
									let { subject, data } = msg;
									// this.intake.emit(subject, data, { subject, socket, rpc : this });
									this.asyncSubscribers.post(subject, { data, socket, ip });
								}
								catch(ex) {
							...
							case 'rpc.req': {
								...
								if( !this.checkAuth(session.user, subject, data, FlowHttp.METHODS.REQUEST) ){
									emit('rpc.resp', { rid, error: "Access Denied" });
									return;
								}

								try {
									if(!subject || subject == 'init-http') {
										emit('rpc.resp', { rid, error: "Malformed request" });
									} else {
										...
										/*
										var listeners = this.intake.listeners(subject);
					...
					else if(websocketMode == FlowHttp.MODE.NATS || websocketMode == 'NATS')
					{
						switch (event) {
							case 'response': {
								let { resp, rid } = msg;
							...
							case 'publish': {
								...
								if(!this.checkAuth(session.user, subject, data, FlowHttp.METHODS.PUBLISH)) {
									emit('publish::response', { rid, error: "Access Denied" });
									return;
								}
								...
								emit('publish::response', { rid, ack : true });
								break;
							}
							case 'unsubscribe': {
								// TODO - sanity checks
								if(!msg || !msg.req) {
									emit('unsubscribe::response', { rid, error : 'malformed request' });
									return;
								}
								...
								if(!sub) {
									emit('unsubscribe::response', { rid, error : 'no such token' });
									return;
								}
								...
								emit('unsubscribe::response', { rid, ok : true });
								break;
							}
							// NATS subscribe
							case 'subscribe': {
								...
								if(!msg || !msg.req || !msg.req.subject) {
									emit('subscribe::response', { rid, error : 'malformed request' });
									return;
								}
								...
								if(!this.checkAuth(session.user, subject, d_, FlowHttp.METHODS.SUBSCRIBE)) {
									emit('subscribe::response', { rid, error: "Access Denied" });
									return;
								}

								if(subscriptions.length > NAX_SUBSCRIPTIONS) {
									emit('subscribe::response', { rid, error: "Maximum Number of Subscriptions Reached" });
									return;
								}
								...
								//this.nats.subscribe(subject, (msg, reply, subject, sid) => {
									...
									if(msg.reply) {
										...
										const rid = rids++;
										emit('request', { rid, req : { subject, data }});
								...
								emit('subscribe::response', { rid, token, subject });
							...
							case 'request': {
								...
								if(this.messageFilter_ && !this.messageFilter_(subject)) {
									emit('response', { rid, error: "Unknown Message" });
									return;
								}
								...
								if(!data)
									data = { };
								if(!this.checkAuth(session.user, subject, data, FlowHttp.METHODS.REQUEST)) {
									emit('response', { rid, error: "Access Denied" });
									return;
								}
	...
	getIpFromHeaders(headers) {
		return headers['x-real-ip'] || headers['x-client-ip'];
	}
